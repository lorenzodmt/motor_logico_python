...
class MotorLogico:
    def __init__(self):
        ...
        self.REPLACEMENTS = [
            ('<->', '=='),
            ('->', ' <= '),  # P -> Q é equivalente a P <= Q em lógica booleana no Python [cite: 2]
            ('&', ' and '),
            ('|', ' or ')
        ]
    ...
    def normalizar_expressao(self, expressao: str) -> str:
        ...
        # CORREÇÃO: For limitador impede carregamento infinito ao digitar expressões incompletas
        for _ in range(10):
            if ")'" not in expr:
                break
    ...
    def preparar_expressao(self, expressao: str) -> str:
        ...
        # CORREÇÃO: For de segurança substitui o while infinito
        for _ in range(10):
    ...
    def explicar_argumento(self, premissas: list, conclusao: str, eh_valido: bool) -> str:
        ...
        if eh_valido:
            ...
            # CORREÇÃO INTEGRADA: Alterado de 'dicionais' para 'condicionais' para evitar NameError [cite: 19]
            ...
            if "|" in c_norm:
                ...
                if any(p in partes_c for p in p_norm):
                    return f"**Regra Identificada:** Adição (A)\n\n" \
                           f"**Explicação:** Como uma das propriedades já é verdadeira pelas premissas, a introdução de uma disjunção " \
...
# 3 Abas perfeitamente integradas
...
with tab_interpretador:
    ...
    st.write("Digite uma expressão para ver como o Motor realiza a análise léxica.")
    ...
    if st.button("Calcular Equivalência", key="btn_equiv"):
        if e1 and e2:
            try:
                ...
                if sao_equivalentes:
                    st.success("### 🟩 Resposta: Expressões LOGICAMENTE EQUIVALENTES (Contrapositiva Válida)")
                else:
...
with tab_inferencia:
    ...
    with col_btn1:
        if st.button("➕ Adicionar Linha de Entrada"):
            st.session_state.num_premissas += 1
            st.rerun()  # Força o Streamlit a redesenhar as chaves dinâmicas sem travar
    with col_btn2:
